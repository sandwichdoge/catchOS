...
;[esp + 8] = next TCB
    ...
    mov edi, [esp + 4]              ;edi = address of the previous task's "thread control block"
    mov [edi + 16], esp             ;Save ESP for previous task's kernel stack in the thread's TCB
 
    ;Load next task's state
 
    mov esi, [esp + 8]              ;esi = address of the next task's "thread control block" (parameter passed on stack)
