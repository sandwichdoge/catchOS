 ...
 */
private void cpuid(int code, unsigned int *a, unsigned int *d) {
    asm volatile("cpuid":"=a"(*a),"=d"(*d):"a"(code):"ecx","ebx");
}
 
/** issue a complete request, storing general registers output as a string
 */
private int cpuid_string(int code, unsigned int where[4]) {
    asm volatile("cpuid":"=a"(*where),"=b"(*(where+1)),
               "=c"(*(where+2)),"=d"(*(where+3)):"a"(code));
    return (int)where[0];
}

public unsigned int cpuid_getcpufeatures(unsigned int *edx_out) {
    unsigned int eax;
    cpuid(CPUID_GETFEATURES, &eax, edx_out);
    return eax;
}

public int cpuid_hasapic() {
    unsigned int edx = 0;
