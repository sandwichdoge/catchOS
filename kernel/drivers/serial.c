...
// baud = 115200 / divisor
public void serial_configure_baud_rate(unsigned short com, unsigned short divisor) {
    outb(SERIAL_LINE_COMMAND_PORT(com), SERIAL_LINE_ENABLE_DLAB);
    outb(SERIAL_DATA_PORT(com), (divisor >> 8) & 0x00ff); // High 8 bits first
    outb(SERIAL_DATA_PORT(com), divisor & 0x00ff);
}

public void serial_configure_line(unsigned short com, unsigned char conf) {
    /*
     Bit:     | 7 | 6 | 5 4 3 | 2 | 1 0 |
...
public void serial_configure_buffer(unsigned short com, unsigned char conf) {
...
public void serial_configure_modem(unsigned short com, unsigned char conf) {
...
public int serial_is_transmit_fifo_empty(unsigned int com) {
    // 0x20 = 0010 0000
    return inb(SERIAL_LINE_STATUS_PORT(com) & 0x20);
}

// Spinning as long as the internal fifo isn't empty, then writing data to data I/O port
public void serial_write(unsigned short com, char *data, unsigned int len) {
...
public void serial_defconfig(unsigned short com) {
